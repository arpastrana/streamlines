...
class Streamsystem():
    ...
    def make_streamlines_jobard_2(self, strat_f, o_prox, st_o_prox, s_prox, num_samples, ite):
        # 1. create queue list with streamlines. store them as they appear
        queue = []
        heapq.heapify(queue)
        min_sp = self.get_max_face_spacing() * strat_f
        ...
        # 4. append first streamline to the queue
        print('******* system streamlines are pushed to the queue *********')
        for idx, strm in enumerate(self.streamlines):
            heapq.heappush(queue, (idx, strm))
    ...
    def make_streamlines_fungi(self, le, s_factor, iterations):
        ...
        for it in range(iterations):
            ...
            for key, value in dict.items():
                ...
                new_nodes = []
                flags = []
                other_streamlines = [v for k, v in dict.items() if k != key]
                others = []
            ...
            # make search tree
                ...
                # split new nodes according to flags
    ...
    def make_sampling_nodes(self, strm, threshold_sp, length=0.1, heap=True):
